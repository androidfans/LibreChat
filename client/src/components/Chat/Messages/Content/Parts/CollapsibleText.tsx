...
const CollapsibleText = memo(({ text, isCreatedByUser, isMarkdown = false }: CollapsibleTextProps) => {
  ...
  useEffect(() => {
    const handleSelectionChange = () => {
      ...
      setIsSelecting(selection ? selection.toString().length > 0 : false);
    };
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (wasExpanded && containerRef.current) {
      setTimeout(() => {
        ...
        let messageContainer = containerRef.current?.closest('.message-render');
        ...
        <div className="absolute bottom-0 left-0 right-0 flex justify-center">
          <button
            ...
            className={cn(
              ...
              'shadow-sm border border-border-light',
          ...
          >
            <ChevronDown
              ...
              aria-hidden="true"
            />
            <span className="text-[10px]">{isExpanded ? localize('com_ui_collapse') : localize('com_ui_expand')}</span>
          </button>
        </div>
      )}
    </div>
